...
import{  getvoucher,createvoucher,deletevoucher,editvoucher}from '../../../services/oderSevice';
...
createUser=async(data)=>{
  try {
    let response=await createvoucher(data);
    if(response)
    {
        this.setState({
...
try {
   ...
   let rs= await deletevoucher(user.id);
    this.handegetall();
    if(rs.errCode===0){
        toast.success('Thành Công', {autoClose:3000})  
    }
