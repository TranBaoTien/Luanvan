...
import{     getPrice,createprice,deletePrice,editPrice}from '../../../services/productService';
...
createUser=async(data)=>{
  try {
    let response=await createprice(data);
    if(response)
    {
        this.setState({
...
try {
   ...
   let res= await deletePrice(user.id);
    this.handegetall();
} catch (e) {
    console.log(e);
}
